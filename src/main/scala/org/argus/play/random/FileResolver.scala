...
object FileResolver {
  ...
  def main(args: Array[String]):  Unit = {
    ...
    var exeSQL : MSet[Context] = msetEmpty
    ...
    val DefsiteMarkMap: MMap[Any, Any] = mmapEmpty
        ...
        apk.model.getEnvMap.get(iComponents) match {
          case Some((esig, _)) =>
            ...
            idfg.icfg.nodes foreach {
              ...
              case cn: ICFGCallNode if cn.getCalleeSig.getSubSignature == "openFileOutput:(Ljava/lang/String;I)Ljava/io/FileOutputStream;"=>
                ...
                for(url <- self;
                  ...
                  if(urlvalue.isInstanceOf[PTAConcreteStringInstance]){
                    val urlvaluestring = "/data/data/" + iComponents.getPackageName + "/files/" + urlvalue.asInstanceOf[PTAConcreteStringInstance].string
              ...
              case cn: ICFGCallNode if cn.getCalleeSig == new Signature("Landroid/database/sqlite/SQLiteOpenHelper;.<init>:(Landroid/content/Context;Ljava/lang/String;Landroid/database/sqlite/SQLiteDatabase$CursorFactory;I)V") =>
                ...
                for(urlvalue <- urlvalues){
                  ...
                  }else if (urlvalue.isInstanceOf[PTAPointStringInstance])
                  {
                    val urlvaluestring2 = urlvalue.asInstanceOf[PTAPointStringInstance].toString()
                    DB_Name += urlvaluestring2
                  }else if (urlvalue.isInstanceOf[Instance]) {
                    val urlvaluestring3 = urlvalue.asInstanceOf[PTAInstance].toString()
                    DB_Name += urlvaluestring3
                  }
                  else{
                    val urlvaluestring4 = urlvalue.asInstanceOf[Instance].defSite.toString()
                    val Loc =  urlvalue.asInstanceOf[Instance].defSite.getCurrentLocUri
                    DB_Name += urlvaluestring4 + Loc
                  }
                }
              case cn: ICFGCallNode if cn.getCalleeSig == new Signature("Landroid/database/sqlite/SQLiteDatabase;.openOrCreateDatabase:(Ljava/lang/String;Landroid/database/sqlite/SQLiteDatabase$CursorFactory;)Landroid/database/sqlite/SQLiteDatabase;") =>
              ...
              case cn: ICFGCallNode if cn.getCalleeSig == new Signature("Landroid/database/sqlite/SQLiteDatabase;.execSQL:(Ljava/lang/String;)V") =>
                ...
                val urlvalues = idfg.ptaresult.pointsToSet(cn.getContext , strSlot)
                for(urlvalue <- urlvalues){
                  var urlvaluestringexecSQL : String = ""
                  if(urlvalue.isInstanceOf[PTAConcreteStringInstance]){
                    urlvaluestringexecSQL = urlvalue.asInstanceOf[PTAConcreteStringInstance].string
                  }else if (urlvalue.isInstanceOf[PTAPointStringInstance])
                  {
                    urlvaluestringexecSQL = urlvalue.asInstanceOf[PTAPointStringInstance].toString()
                  }else if (urlvalue.isInstanceOf[Instance]) {
                    urlvaluestringexecSQL = urlvalue.asInstanceOf[PTAInstance].toString()
                  }
                  else{
                    urlvaluestringexecSQL = urlvalue.asInstanceOf[Instance].defSite.toString()
                  }
                  if(urlvaluestringexecSQL.split("[(]")(0).trim().split("[ ]")(0).trim().equalsIgnoreCase("create")){
                    val structure = urlvaluestringexecSQL.split("[(]")(1).trim().split("[)]")(0).trim()
                    val TBName = urlvaluestringexecSQL.split("[(]")(0).trim().split("[ ]").last.trim()
                    IndexMap2.getOrElseUpdate(TBName, structure)
                  }else if (urlvaluestringexecSQL.split("[(]")(0).trim().split("[ ]")(0).trim().equalsIgnoreCase("insert") |
                    urlvaluestringexecSQL.split("[(]")(0).trim().split("[ ]")(0).trim().equalsIgnoreCase("update") ){
                    val TBName = urlvaluestringexecSQL.split("[(]")(0).trim().split("[ ]").last.trim()
                    test7 += cn.context
                    exeSQL += cn.context
                    val strSlot = VarSlot(cn.argNames(1))
              ...
              case cn: ICFGCallNode if cn.getCalleeSig == new Signature("Landroid/os/Environment;.getExternalStorageDirectory:()Ljava/io/File;") =>
                test3 += cn.context
              case cn: ICFGCallNode if cn.getCalleeSig.methodName == new Signature("Landroid/content/Context;.getPackageName:()Ljava/lang/String;").methodName =>
                      ...
                      case cn: ICFGCallNode if cn.getContext == M2 && cn.getCalleeSig.getSubSignature == "getSharedPreferences:(Ljava/lang/String;I)Landroid/content/SharedPreferences;"=>
                        ...
                        for(urlvalue <- urlvalues) {
                          ...
                          if(urlvalue.isInstanceOf[PTAConcreteStringInstance]){
                            ...
                            val urlvaluestring = "data/data/" + a + "/shared_prefs/" + urlvalue.asInstanceOf[PTAConcreteStringInstance].string + ".xml"
                            IndexMap0(urlvalue.defSite.getCurrentLocUri) = urlvaluestring
                          }else if (urlvalue.isInstanceOf[Instance])
                          {
                            val urlvaluedef = urlvalue.asInstanceOf[Instance].defSite
                            IndexMap0(urlvalue.defSite.getCurrentLocUri) = urlvaluedef
                            DefsiteMarkMap.update(M2K , 2)
                          }else{
                            val urlvalueelse = "ANY"
                            IndexMap0(urlvalue.defSite.getCurrentLocUri) = urlvalueelse
                          }
            ...
            urlMapPlus.keys.foreach{
              K =>
                var temp:MList[Any] = mlistEmpty
                temp = urlMapPlus(K).filter(L_member => L_member.isInstanceOf[java.lang.String])
                urlMapPlus(K) = temp
            }
            println("*****************************************************")
            gisNodes.foreach {
              gisnode =>
                if (gisnode.node.node.asInstanceOf[ICFGInvokeNode].getCalleeSig == new Signature("Ljava/io/FileWriter;.write:(Ljava/lang/String;)V")
                  ...
                  connValues foreach {
                    urlValue =>
                      ...
                      }else if(DefsiteMarkMap.keySet.contains(urlValue.defSite)) {
                        DefsiteMarkMap.keys.foreach{
                          DefsiteMarkK =>
                            if ((urlValue.defSite == DefsiteMarkK)&&(DefsiteMarkMap(DefsiteMarkK) == 1)){
                              println("File's name at " + gisnode.descriptor + "@" + gisnode.node.node.getContext.getLocUri + "\nis:\n" + "/data/data/" + iComponents.getPackageName + "/files/" + urlMapPlus.getOrElse(urlValue.defSite, mlistEmpty).mkString("\n"))
                            }else if ((urlValue.defSite == DefsiteMarkK)&&(DefsiteMarkMap(DefsiteMarkK) == 2)){
                              println("File's name at " + gisnode.descriptor + "@" + gisnode.node.node.getContext.getLocUri + "\nis:\n" + "/data/data/" + iComponents.getPackageName + "/shared_prefs/" + urlMapPlus.getOrElse(urlValue.defSite, mlistEmpty).mkString("\n") + ".xml")
                            }
                            else {
                              println("File's name at " + gisnode.descriptor + "@" + gisnode.node.node.getContext.getLocUri + "\nis:\n" + urlMapPlus.getOrElse(urlValue.defSite, mlistEmpty).mkString("\n") + ".xml")
                            }
                ...
                if (gisnode.node.node.asInstanceOf[ICFGInvokeNode].getCalleeSig == new Signature("Landroid/database/sqlite/SQLiteDatabase;.execSQL:(Ljava/lang/String;)V")){
                  val invNode = gisnode.node.node.asInstanceOf[ICFGInvokeNode]
                  val exeEQLcon = invNode.getContext
                  if (exeSQL.contains(exeEQLcon)){
